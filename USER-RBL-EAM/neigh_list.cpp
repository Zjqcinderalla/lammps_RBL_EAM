...
NeighList::NeighList(LAMMPS *lmp) : Pointers(lmp)
{
  ...
  //--------zjq----------//
  neigh_rbl = rbl;
  // neigh_cut_core_off = cut_core_off;
  // 
  numneigh_core = NULL;
  firstneigh_core = NULL;
  ipage_core = NULL;
  numneigh_shell = NULL;
  firstneigh_shell = NULL;
  ipage_shell = NULL;
  // 
  secondnumneigh_shell = NULL;
  secondneigh_shell = NULL;
  ipage_shell_second = NULL;
...
NeighList::~NeighList()
{
  ...
  if (!copy) {
    ...
    delete [] ipage;
    //------------zjq----------------//
    // printf("neighbor_list:%d \n", neigh_rbl);
    if (neigh_rbl){
      memory->destroy(numneigh_core);
      memory->sfree(firstneigh_core);
      delete [] ipage_core;
      memory->destroy(numneigh_shell);
      memory->sfree(firstneigh_shell);
      delete [] ipage_shell;
      // 
      memory->destroy(secondnumneigh_shell);
      memory->sfree(secondneigh_shell);
      delete [] ipage_shell_second;
    }    
    //-----------zjq-----------------//
  }
...
void NeighList::setup_pages(int pgsize_caller, int oneatom_caller)
{
  ...
  // ------------zjq------------------- //
  // printf("setup_pages:%d \n", neigh_rbl);
  if (neigh_rbl){
    ipage_core = new MyPage<int>[nmypage];
    for (int i = 0; i < nmypage; i++)
    ipage_core[i].init(oneatom,pgsize,PGDELTA);
    // 
    ipage_shell = new MyPage<int>[nmypage];
    for (int i = 0; i < nmypage; i++)
    ipage_shell[i].init(oneatom,pgsize,PGDELTA);
    //
    ipage_shell_second = new MyPage<int>[nmypage];
    for (int i = 0; i < nmypage; i++)
    ipage_shell_second[i].init(oneatom,pgsize,PGDELTA);
  } 
  // ---------------zjq--------------- //
...
void NeighList::grow(int nlocal, int nall)
{
  ...
  if (neigh_rbl){
    memory->destroy(numneigh_core);
    memory->sfree(firstneigh_core);
    memory->create(numneigh_core,maxatom,"neighlist:numneigh_core");
    firstneigh_core = (int **) memory->smalloc(maxatom*sizeof(int *),
                                            "neighlist:firstneigh_core");
    memory->destroy(numneigh_shell);
    memory->sfree(firstneigh_shell);
    memory->create(numneigh_shell,maxatom,"neighlist:numneigh_shell");
    firstneigh_shell = (int **) memory->smalloc(maxatom*sizeof(int *),
                                            "neighlist:firstneigh_shell");
    // 
    memory->destroy(secondnumneigh_shell);
    memory->sfree(secondneigh_shell);
    memory->create(secondnumneigh_shell,maxatom,"neighlist:secondnumneigh_shell");
    secondneigh_shell = (int **) memory->smalloc(maxatom*sizeof(int *),
                                            "neighlist:secondneigh_shell");
...
bigint NeighList::memory_usage()
{
  ...
  if (neigh_rbl){
    bytes += memory->usage(numneigh_core,maxatom);
    bytes += maxatom * sizeof(int *);
    if (ipage_core) {
      for (int i = 0; i < nmypage; i++)
        bytes += ipage_core[i].size();
    }
    // 
    bytes += memory->usage(numneigh_shell,maxatom);
    bytes += maxatom * sizeof(int *);
    if (ipage_shell) {
      for (int i = 0; i < nmypage; i++)
        bytes += ipage_shell[i].size();
    }
    // 
    bytes += memory->usage(secondnumneigh_shell,maxatom);
    bytes += maxatom * sizeof(int *);
    if (ipage_shell_second) {
      for (int i = 0; i < nmypage; i++)
        bytes += ipage_shell_second[i].size();
    }
