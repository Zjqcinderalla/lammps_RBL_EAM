...
double times_verlet_force;
// times_eam_force;
double times_rho, times_phi;
...
LAMMPS::~LAMMPS()
{
  ...
  if ((me == 0) && (screen || logfile)) {
    char outtime[128];
    char outtime_zjq_total[128];
    char outtime_zjq_verlet[128];
    char outtime_zjq_rho[128];
    char outtime_zjq_phi[128];

    sprintf(outtime_zjq_total,"Times of total computation："
            "%lf\n", totalclock);
    sprintf(outtime_zjq_verlet,"Times of verlet_force computation："
            "%lf\n", times_verlet_force);
    sprintf(outtime_zjq_rho,"Times of force-rho computation："
            "%lf\n", times_rho);
    sprintf(outtime_zjq_phi,"Times of force-phi computation："
            "%lf\n", times_phi);
    ...
    if (screen) fputs(outtime_zjq_total,screen);
    if (screen) fputs(outtime_zjq_verlet,screen);
    if (screen) fputs(outtime_zjq_rho,screen);
    if (screen) fputs(outtime_zjq_phi,screen);
    
    if (logfile) fputs(outtime_zjq_total,logfile);
    if (logfile) fputs(outtime_zjq_verlet,logfile);
    if (logfile) fputs(outtime_zjq_rho,logfile);
    if (logfile) fputs(outtime_zjq_phi,logfile);
  }
