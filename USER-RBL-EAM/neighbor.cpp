...
void Neighbor::init()
{
  ...
  if (cutneighsq == NULL) {
    if (lmp->kokkos) init_cutneighsq_kokkos(n);
      ...
      // memory->create(cutcoresq,n+1,n+1,"neigh:cutcoresq");
    }
  ...
  // zjq
  if (cutcoresq == NULL) memory->create(cutcoresq,n+1,n+1,"neigh:cutcoresq");

  double cutoff,delta,cut,cut_core_off_local,cut_core;
  cutneighmin = BIG;
  cutneighmax = 0.0;
  //-------------------zjq--------------------//
  cut_core_off_local = cut_core_off;
  ...
  for (i = 1; i <= n; i++) {
    ...
    for (j = 1; j <= n; j++) {
      ...
      cut = cutoff + delta;
      cut_core = cut_core_off_local + delta;

      // 
      cutneighsq[i][j] = cut*cut;
      cutcoresq[i][j] = cut_core*cut_core;
